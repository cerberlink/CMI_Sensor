...
Sources
    - https://www.kaggle.com/competitions/cmi-detect-behavior-with-sensor-data/discussion/583118
"""
...
def prepare_sequence_data(df, sequence_id):
    ...
    for _, row in sequence_df.iterrows():
        # Convert to scalar-last quaternion [x, y, z, w] for scipy
        quat = [float(row["rot_x"]), float(row["rot_y"]), float(row["rot_z"]), float(row["rot_w"])]
